...
2. change-scene：创建一个新场景。需要提供完整的场景信息（id、title、summary、currentSituation、time、location、activeCharacters），beat 字段为单个入口节拍对象。
...
## 输出字段说明
- id：字符串，场景唯一标识（change-scene 时必填；insert-beat 可沿用当前场景 id）
- title：字符串，场景标题（change-scene 时必填）
- summary：字符串，场景摘要（change-scene 时必填，100字以内）
- currentSituation：字符串，当前局势描述（change-scene 时必填）
- time：字符串，时间
- location：字符串，地点
